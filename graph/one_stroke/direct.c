...
void connect_direct(int idx1, int idx2, int root_num) {
    if (idx1 < 0 || idx1 >= SIZE)
        return;
    if (idx2 < 0 || idx2 >= SIZE)
        return;
...
void setup()
{
    ...
    connect_direct(0, 1, 1);
    connect_direct(1, 2, 1);
    connect_direct(1, 3, 1);
    connect_direct(2, 3, 1);
    connect_direct(3, 0, 1);
    connect_direct(3, 1, 1);
}
